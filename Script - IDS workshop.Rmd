...
# Wrangling data at scale with data.table `r emo::ji(':wrench:')`
...
###   j, # where j indicates "what to do"
###   by] # where by means "grouped by what?"

## Now, let's wrangle that data! `r emo::ji(':computer:')`
...
inrange_vector # 3,182 rows, all but one student from the dataset, who falls outside range
```

## Extracting or modifying columns: 
### Tidyverse equivalent: select
### The logic is fairly straightforward. The main thing to keep in mind is, that if we don't wrap j, R will return a vector. If we want a data.table instead (most of the time this is what we want!) we need to wrap j, either with list() or with .(). Generally, you will see .() more commonly as its shorter. 
...
## Full-on data.table - Advanced operations: `r emo::ji(':rocket:')` 
